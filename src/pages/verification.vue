  ...
  <div v-if="$q.platform.is.mobile" class="mobile-window-height row justify-center items-center">
    <div class="q-gutter-md" style="width: 100%">
      <div class="row justify-center">
        <q-icon class="text-grey" name="o_domain_verification" size="100px"></q-icon>
      </div>
      <div class="text-h5 text-grey text-center">Verification Page</div>
    </div>
  </div>
    ...
    router.isReady().then(() => {
      ...
      watch(() => store.busy, (newValue) => {
        if (newValue && store.loading) {
          Loading.show({
            message: 'Sending data for verification in please wait...',
      ...
      const id = typeof route.params.verification_id === 'string' ? route.params.verification_id : ''
      store.verification(code, id)
    }).catch()
  }
