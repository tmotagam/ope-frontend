...
<template>
  <div v-if="!verificationStatus">
    <div v-for="moderator in store.moderatorsdata" v-bind:key="moderator.Id">
      <br />
      <q-item class="text-teal-14" clickable @click.stop="showDetail(moderator)" v-ripple>
        <q-item-section avatar top>
          <q-icon name="o_how_to_reg" color="teal-14" size="34px" />
        </q-item-section>

        <q-item-section top class="col-2 gt-sm">
          <q-item-label class="q-mt-sm">{{ moderator.id }}</q-item-label>
        </q-item-section>
        ...
        <q-item-section top>
          ...
          <q-item-label lines="1">
            <span class="text-weight-medium">{{ moderator.Id }}</span>
          </q-item-label>
          <q-item-label caption lines="1">Pending Verification of moderator {{ moderator.name }}</q-item-label>
        </q-item-section>

        <q-item-section top side>
          <div class="text-grey-8 q-gutter-xs">
            <q-btn class="gt-xs" size="17px" color="red-12" flat @click.stop="deleteuser(moderator.Id)" dense round
              icon="o_delete">
              <q-tooltip class="bg-white text-black" style="font-size: 15px">Delete moderator
              </q-tooltip>
            </q-btn>
          </div>
        </q-item-section>
  ...
  <div v-if="verificationStatus">
    <div v-for="moderator in store.moderatorsdata" v-bind:key="moderator.Id">
      <br />
      <q-item class="text-positive" clickable @click.stop="vshowDetail(moderator)" v-ripple>
        <q-item-section avatar top>
        ...
        <q-item-section top>
          <q-tooltip class="bg-white text-black" style="font-size: 15px">Click to view {{ moderator.name }}`s profile
          </q-tooltip>
          <q-item-label lines="1">
            <span class="text-weight-medium">{{ moderator.Id }}</span>
          </q-item-label>
          <q-item-label caption lines="1">Verified moderator {{ moderator.name }}</q-item-label>
        </q-item-section>
    ...
    <q-card v-if="!store.mainstore.busy" class="text-black">
      ...
      <q-card-section>
        <div class="q-gutter-xl flex flex-center row justify-center items-center">
          <q-img fit="contain" width="500px" height="500px" :src="'data:image/png;base64,' + store.moderatorsimages[0]">
          ...
          <q-img fit="contain" width="500px" height="500px" :src="'data:image/png;base64,' + store.moderatorsimages[1]">
            <q-icon class="absolute all-pointer-events" size="32px" name="info" color="info" style="top: 8px; left: 8px">
              <q-tooltip class="bg-white text-black" style="font-size: 15px">
      ...
      <q-card-actions align="center">
        <q-btn class="q-mr-lg" label='Accept the moderator' style="font-size: 120%" push color="white"
          text-color="positive" @click.stop="store.verify_moderator(detail.Id, verificationStatus)" v-close-popup />
        <q-btn class="q-ml-lg" label='Reject the moderator' style="font-size: 120%" push color="white"
...
export default defineComponent({
  name: 'MverificationLists',

  setup(props) {
    const store = AdminStore()
    store.moderators_details(props.verificationStatus)
    watch(() => store.moderatorsidsstatus.ids.length, () => {
      if (!store.eventinprogress) store.moderators_details(props.verificationStatus)
    })
    if (props.verificationStatus === true) {
      watch(() => store.vmoderatorCurrent, () => {
        store.moderators_details(props.verificationStatus)
      })
    } else {
      watch(() => store.pmoderatorCurrent, () => {
        store.moderators_details(props.verificationStatus)
      })
    }
    return {
  ...
  methods: {
    showDetail(moderator: {
      id: number;
      Id: string;
      name: string;
    }) {
      this.detail = moderator
      this.store.moderator_images(moderator.Id)
      this.dialog = true
    },
    vshowDetail(moderator: {
      id: number;
      Id: string;
      name: string;
    }) {
    ...
    rejects(id: string) {
      ...
      this.store.disprove_moderator(id, this.reason, this.problem)
    },
    deleteuser(id: string) {
      Dialog.create({
        ...
        message: `Do you really want to delete moderator ${id} ?`,
        cancel: {
          'text-color': 'primary',
          flat: true,
          label: 'No'
        ...
        .onOk(() => { this.store.delete_moderator(id) })
        .onCancel(() => undefined)
    },
  },
